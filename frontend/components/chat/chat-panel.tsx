...
import { deriveAgentControl } from "@/lib/agent-proposals";
import { ChatMessageView } from "./chat-message";
import { ProposalsRail } from "./proposals-rail";
import {
  Empty,
  EmptyContent,
  EmptyDescription,
    ...
    <div className="flex flex-col h-full bg-card">
      <div className="flex h-10 items-center justify-between shrink-0 border-b border-border px-3">
        <div className="flex items-center gap-1.5 min-w-0">
          ...
          <div className="relative min-w-0" ref={sessionsRef}>
            <button
              ...
              className="flex items-center gap-1 max-w-[180px] text-xs font-semibold tracking-wide text-muted-foreground uppercase hover:text-foreground transition-colors"
            >
            ...
            {sessionsOpen && (
              <div className="absolute left-0 top-full z-50 mt-1 w-56 rounded-md border border-border bg-popover shadow-md py-1 max-h-48 overflow-y-auto">
                {sessions.length === 0 && (
                  <p className="px-2.5 py-1.5 text-xs text-muted-foreground">No saved sessions</p>
                )}
                {sessions.map((s) => (
                  <button
                    key={s.id}
                    ...
                    className={cn(
                      "w-full text-left px-2.5 py-1.5 text-sm hover:bg-accent transition-colors",
        ...
        <button
          ...
          className="flex items-center gap-1 px-1.5 py-1 rounded-sm text-xs transition-colors hover:bg-accent text-muted-foreground shrink-0"
        >
      ...
      {anthropicConfigured === false && (
        <div className="shrink-0 border-b border-[color-mix(in_srgb,var(--attention)_25%,var(--border))] bg-[color-mix(in_srgb,var(--attention)_8%,var(--card))] px-3 py-2 text-xs leading-snug text-[var(--attention)]">
          Add your Anthropic API key to <code className="rounded bg-black/8 px-1 py-0.5 font-mono text-xs">backend/.env</code> as{" "}
          <code className="rounded bg-black/8 px-1 py-0.5 font-mono text-xs">ANTHROPIC_API_KEY</code>, then restart the app. See{" "}
          <code className="font-mono text-xs">backend/API_KEYS.md</code>.
        </div>
      )}
      ...
      <div className="px-2.5 pb-2 pt-1.5 shrink-0 border-t border-border">
        <div className="flex items-end gap-1.5 rounded-[3px] px-2 py-1 border border-border bg-background focus-within:border-ring/50 transition-colors">
          <textarea
            ...
            className={cn(
              "flex-1 resize-none bg-transparent text-sm outline-none min-h-[20px] max-h-[120px] leading-relaxed",
...
function EmptyState({ onSuggest }: { onSuggest: (s: string) => void }) {
  ...
  return (
    <Empty className="h-full rounded-none border-0 py-6">
      <EmptyHeader>
        <EmptyMedia variant="icon">
          <Icon name="auto_awesome" size={16} />
        </EmptyMedia>
        <EmptyTitle>Propose, then you decide</EmptyTitle>
        <EmptyDescription>
          The agent shows a proposal before it writes. Approve or deny each change.
        </EmptyDescription>
      </EmptyHeader>
      <EmptyContent>
        <div className="flex w-full flex-col gap-1">
          {suggestions.map((s) => (
            <button
              ...
              type="button"
              onClick={() => onSuggest(s)}
              className="w-full rounded-[3px] border border-border px-2.5 py-1.5 text-left text-xs text-muted-foreground hover:border-primary/40 hover:bg-accent"
            >
              {s}
            </button>
          ))}
